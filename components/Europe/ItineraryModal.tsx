...
import { X, ChevronLeft, ChevronRight, Sun, Sunset, Moon, MapPin, Clock, Calendar, Compass, Heart, Users, UserPlus, Mountain, Gem, Plane, Train, Download, AlertTriangle, Snowflake, Flower2, TreeDeciduous } from 'lucide-react'
import { TripType, Season, calculateDistance, getCitySeasonalCompatibility, getSeasonInfo, SeasonCompatibility } from './europeData'
...
const compatibilityColors: Record<SeasonCompatibility, string> = {
...
export default function ItineraryModal({ selectedCities, tripType, season, onClose }: ItineraryModalProps) {
  const [currentDayIndex, setCurrentDayIndex] = useState(0)
  const [isGeneratingPDF, setIsGeneratingPDF] = useState(false)
  
  // Generate the full itinerary with per-city trip types and season
  const itinerary = generateFullItinerary(
    selectedCities.map(sc => ({ 
      cityId: sc.city.id, 
      nights: sc.nights,
      tripType: sc.tripType ?? tripType
    })),
    tripType,
    season
  )
  ...
  // Get seasonal warnings for selected cities
  const seasonalWarnings = season ? selectedCities.map(sc => {
    const compatibility = getCitySeasonalCompatibility(sc.city.id, season)
    return {
      city: sc.city.name,
      ...compatibility
    }
  }).filter(w => w.warning || w.compatibility === 'limited' || w.compatibility === 'not-recommended') : []
  ...
  const handleDownloadItinerary = async () => {
    ...
    try {
              ...
              const activitiesHTML = timeActivities
                .map((activity) => {
                  ...
                  const specialBadge = activity.tripTypes
                    ...
                    : ''
                  const seasonBadge = activity.seasons
                    ? `<span class="season-badge">${activity.seasons.map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(', ')}</span>`
                    : ''
                  return `
      ...
      const seasonHTML = seasonInfo ? `
        <div class="season-info">
          <span class="season-icon">${seasonInfo.icon}</span>
          <span class="season-name">${seasonInfo.name} Trip</span>
          <span class="season-months">(${seasonInfo.months})</span>
        </div>
      ` : ''
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Inter', sans-serif; background: var(--white); color: var(--text); line-height: 1.6; font-size: 11pt; }
    .container { max-width: 800px; margin: 0 auto; padding: 40px 30px; }
    
    .cover { text-align: center; padding: 60px 20px; background: linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%); color: var(--white); margin: -40px -30px 40px -30px; page-break-after: always; }
    .cover-logo { font-size: 48px; margin-bottom: 20px; }
    .cover h1 { font-family: 'Playfair Display', serif; font-size: 36pt; font-weight: 700; margin-bottom: 10px; }
    .cover h1 span { color: var(--accent); font-style: italic; }
    .cover-subtitle { font-size: 14pt; opacity: 0.9; margin-bottom: 30px; }
    
    .season-info { display: inline-flex; align-items: center; gap: 8px; background: rgba(255,255,255,0.2); padding: 8px 16px; border-radius: 20px; margin-bottom: 20px; }
    .season-icon { font-size: 20px; }
    .season-name { font-weight: 600; }
    .season-months { opacity: 0.8; font-size: 12pt; }
    
    .cover-stats { display: flex; justify-content: center; gap: 40px; margin-top: 40px; }
    .stat { text-align: center; }
    .stat-value { font-family: 'Playfair Display', serif; font-size: 28pt; font-weight: 700; color: var(--accent); }
    .stat-label { font-size: 10pt; text-transform: uppercase; letter-spacing: 2px; opacity: 0.8; }
    .cover-date { margin-top: 40px; font-size: 10pt; opacity: 0.7; }
    
    .overview { background: var(--bg); border-radius: 12px; padding: 24px; margin-bottom: 30px; }
    .overview h2 { font-family: 'Playfair Display', serif; font-size: 18pt; color: var(--text-dark); margin-bottom: 16px; }
    .route { font-size: 11pt; color: var(--text); line-height: 1.8; }
    
    .day-card { background: var(--white); border: 1px solid rgba(18, 16, 61, 0.1); border-radius: 12px; margin-bottom: 24px; overflow: hidden; page-break-inside: avoid; }
    .day-header { background: linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%); color: var(--white); padding: 20px 24px; }
    .day-info { display: flex; align-items: center; gap: 12px; margin-bottom: 8px; flex-wrap: wrap; }
    .day-number { background: rgba(255,255,255,0.2); padding: 4px 12px; border-radius: 20px; font-size: 10pt; font-weight: 600; }
    .day-city { font-size: 10pt; opacity: 0.9; }
    .trip-badge { background: var(--accent); color: var(--white); padding: 4px 10px; border-radius: 20px; font-size: 9pt; font-weight: 600; }
    .day-title { font-family: 'Playfair Display', serif; font-size: 16pt; font-weight: 600; }
    .day-content { padding: 20px 24px; }
    
    .time-section { margin-bottom: 20px; }
    .time-section:last-child { margin-bottom: 0; }
    .time-header { display: inline-flex; align-items: center; gap: 8px; padding: 6px 14px; border-radius: 20px; font-size: 10pt; font-weight: 500; margin-bottom: 12px; border: 1px solid; }
    .time-header.morning { background: var(--morning); color: var(--morning-text); border-color: #fcd34d; }
    .time-header.afternoon { background: var(--afternoon); color: var(--afternoon-text); border-color: #fdba74; }
    .time-header.evening { background: var(--evening); color: var(--evening-text); border-color: #a5b4fc; }
    .time-icon { font-size: 14px; }
    
    .activities-list { border-left: 2px solid rgba(18, 16, 61, 0.1); margin-left: 16px; padding-left: 16px; }
    .activity { position: relative; padding: 12px 0; border-bottom: 1px solid rgba(18, 16, 61, 0.05); }
    .activity:last-child { border-bottom: none; }
    .activity::before { content: ''; position: absolute; left: -21px; top: 18px; width: 10px; height: 10px; border-radius: 50%; background: var(--white); border: 2px solid rgba(18, 16, 61, 0.3); }
    .special-activity { background: linear-gradient(90deg, rgba(209, 148, 87, 0.05) 0%, transparent 100%); padding: 12px; margin: 8px 0; border-radius: 8px; }
    .special-activity::before { background: var(--accent); border-color: var(--accent); }
    .activity-header { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; margin-bottom: 6px; }
    .activity-title { font-family: 'Playfair Display', serif; font-size: 12pt; font-weight: 600; color: var(--text-dark); }
    .special-badge { background: var(--accent); color: var(--white); padding: 2px 8px; border-radius: 12px; font-size: 8pt; font-weight: 600; }
    .season-badge { background: #e0f2fe; color: #0369a1; padding: 2px 8px; border-radius: 12px; font-size: 8pt; font-weight: 600; }
    .duration { color: var(--text); font-size: 9pt; }
    .duration::before { content: '⏱ '; }
    .activity-description { font-size: 10pt; color: var(--text); line-height: 1.6; }
    
    .footer { text-align: center; padding: 30px 20px; margin-top: 40px; border-top: 1px solid rgba(18, 16, 61, 0.1); }
    .footer-logo { font-family: 'Playfair Display', serif; font-size: 14pt; color: var(--text-dark); margin-bottom: 8px; }
    .footer-logo span { color: var(--accent); font-style: italic; }
    .footer-text { font-size: 9pt; color: var(--text); }
    
    @media print {
      body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      .container { padding: 20px; }
      .cover { margin: -20px -20px 30px -20px; }
      .day-card { page-break-inside: avoid; break-inside: avoid; }
      .no-print { display: none !important; }
    }
    
    .download-bar { position: fixed; top: 0; left: 0; right: 0; background: var(--primary); padding: 12px 20px; display: flex; justify-content: center; gap: 16px; z-index: 100; }
    .download-btn { background: var(--accent); color: var(--white); border: none; padding: 10px 24px; border-radius: 25px; font-family: 'Inter', sans-serif; font-size: 12pt; font-weight: 600; cursor: pointer; transition: all 0.2s; }
    .download-btn:hover { background: var(--accent-dark); transform: scale(1.02); }
    .download-btn.secondary { background: transparent; border: 2px solid var(--white); }
    .download-btn.secondary:hover { background: rgba(255,255,255,0.1); }
    @media print { .download-bar { display: none !important; } .container { padding-top: 20px; } }
    @media screen { .container { padding-top: 80px; } }
  </style>
</head>
<body>
  ...
  const SeasonIcon = season ? seasonIcons[season] : null
  const seasonInfo = season ? getSeasonInfo(season) : null

  // Get current city's seasonal compatibility
  const currentCityData = selectedCities.find(sc => sc.city.name === currentDayData.cityName)
  const currentCitySeasonalInfo = season && currentCityData 
    ? getCitySeasonalCompatibility(currentCityData.city.id, season) 
    : null
      ...
      >
        ...
        >
          ...
          <div className="relative h-40 sm:h-48 md:h-56 flex-shrink-0">
            ...
            {/* Badges row */}
            <div className="absolute top-3 sm:top-4 left-3 sm:left-4 flex items-center gap-2 flex-wrap">
              {/* Trip type badge */}
              {currentCityTripType && TripIcon && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#d19457] text-white text-xs sm:text-sm font-medium">
                  <TripIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  {tripTypeLabels[currentCityTripType].name}
                </div>
              )}
              
              {/* Season badge */}
              {season && SeasonIcon && seasonInfo && (
                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium border ${seasonColors[season]}`}>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6">
              ...
              {/* Seasonal highlight for current city */}
              {currentCitySeasonalInfo?.highlight && (
          ...
          {seasonalWarnings.length > 0 && (
            <div className="bg-amber-50 border-b border-amber-200 px-4 sm:px-6 py-2 flex-shrink-0">
              <div className="flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
                <div className="text-xs sm:text-sm text-amber-800">
                  <span className="font-medium">Seasonal Note:</span>{' '}
                  {seasonalWarnings.map((w, i) => (
                    <span key={w.city}>
                      {w.city}: {w.warning}
                      {i < seasonalWarnings.length - 1 && ' • '}
                    </span>
                  ))}
                </div>
              </div>
                        ...
                        {activities.map((activity, actIndex) => (
                          <div 
                            ...
                            className={`relative pl-4 ${
                              activity.tripTypes || activity.seasons
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {activity.seasons && (
                                    <span className={`px-2 py-0.5 text-[10px] sm:text-xs font-medium rounded-full border ${
                                      activity.seasons.includes(season!) ? seasonColors[season!] : 'bg-gray-100 text-gray-600 border-gray-200'
                                    }`}>
                                      {activity.seasons.map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(', ')}
                                    </span>
                                  )}
                                </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div className="flex items-center gap-1 sm:gap-2 overflow-x-auto max-w-full pb-1 sm:pb-0 scrollbar-hide flex-1">
                {selectedCities.map((sc, index) => {
                  ...
                  // Get seasonal compatibility for this city
                  const citySeasonInfo = season ? getCitySeasonalCompatibility(sc.city.id, season) : null
                  ...
                  return (
                    <div key={sc.city.id} className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
                      ...
                      >
                        ...
                        {/* Show warning indicator if city has seasonal warning */}
                        {citySeasonInfo?.warning && (
